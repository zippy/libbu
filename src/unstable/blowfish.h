...
#include "bu/ciphermodecbc.h"
#include "bu/ciphermodeofb.h"
...
#define F(x) \
    (((SB[0][x.byte.zero] + SB[1][x.byte.one]) ^ SB[2][x.byte.two]) + \
     SB[3][x.byte.three])
    ...
    public:
        ...
        void setPassword( const Bu::String &sPass )
        {
            ...
            if (len > 0)
            {
                ...
                {
                    Work.byte.zero = sPass[(j++)%len];
                    Work.byte.one = sPass[(j++)%len];
                    Work.byte.two = sPass[(j++)%len];
                    Work.byte.three = sPass[(j++)%len];
                    PA[i] ^= Work.word;
                }
...
#error No endianness defined
    ...
    typedef CipherModeEcb<8, Blowfish<1> > BlowfishEcb;
    typedef CipherModeCfb<8, Blowfish<1> > BlowfishCfb;
    typedef CipherModeCbc<8, Blowfish<1> > BlowfishCbc;
    typedef CipherModeOfb<8, Blowfish<1> > BlowfishOfb;
};
